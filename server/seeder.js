...
const insertUser=async ()=>{
    ...
    try{
        for(const user of users){
            await pool.query(query, [user.full_name, user.email, user.contact_number, user.gender, user.country, user.region, user.password, user.isHost, user.profile]);
        }
...
const insertHomestays= async ()=>{
    ...
    try {
        for(const homestay of homestays){
            await pool.query(query, [homestay.user_id, homestay.dzongkhag, homestay.gewog, homestay.latitude, homestay.longitude, homestay.facilities, homestay.check_in, homestay.check_out, homestay.rate, homestay.rules, homestay.accomodation, homestay.isAvailable, homestay.images, homestay.numReviews, homestay.rating, homestay.title, homestay.description]);
        }
...
const insertReviews= async ()=>{
    ...
    try {
        for(const review of reviews){
            await pool.query(query, [review.user_id, review.homestay_id, review.review, review.rating]);
        }
